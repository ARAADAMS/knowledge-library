# Audio Speech

9 entries.

- [Nian Shao, Rui Zhou, Pengyu Wang, Xian Li, Ying Fang, Yujie Yang and Xiaofei Li](2502.20040.md)
- [AG-REPA: Causal Layer Selection for Representation Alignment in](2603.01006.md)
- [F.; Lastname, F. Spectrogram features](2603.14917.md)
- [for the deposit and dissemination of scientific re￾search documents, whether they are published or not.](for_the_deposit_and_dissemination_of_scientific_re_search_documents_whether_they.md)
- [Hooking up spectro-temporal filters with auditory-inspired representations for](hooking_up_spectro-temporal_filters_with_auditory-inspired_representations_for.md)
- [ISSN: 3021-940X (Print) DOI: https://doi.org/10.3126/injet.v2i2.78615](issn_3021-940x_print_doi_https_doi.org_10.3126_injet.v2i2.78615.md)
- [The contemporary landscape of speech synthesis is characterized by a reliance on discrete](the_contemporary_landscape_of_speech_synthesis_is_characterized_by_a_reliance_on.md)
- [The core issue seems to be missing linguistic structure rather than sheer capacity. Although a 16k‑bit HV is](the_core_issue_seems_to_be_missing_linguistic_structure_rather_than_sheer_capaci.md)
- [The landscape of artificial intelligence-powered text-to-speech (TTS) generation in 2026](the_landscape_of_artificial_intelligence-powered_text-to-speech_tts_generation_i.md)
